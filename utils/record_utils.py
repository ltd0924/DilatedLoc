...
def list_to_dict(test_csv, pred = False, scale=[100,100,700,10000]):
    test_list = {}
    num = 0
...
def assess(pred_data, test_data, tolerance = 200,tolerance_ax = 500, min_int = 1000,scale=[100,100,700,10000]):
    # time_postprocess = time.time()
    test_list, num = list_to_dict(test_data,scale=scale)
    print('\nevaluation on {0} images,contain ground truth: {1}'.format(len(test_list), num))
    pred_list, num = list_to_dict(pred_data, True,scale=scale)
