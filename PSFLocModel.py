...
class PSFLocModel:
    def __init__(self,setup_params,type):
        ...
        self.DilatedLoc = LocalizationCNN(dilation_flag=setup_params['net_params']['dilation_flag'],
                                          local_context=setup_params['net_params']['local_flag'],
                                          feature=setup_params['net_params']['n_filters']
                                          ).to(torch.device('cuda'))

        # self.DilatedLoc = LocalizationCNN_Unet_depwise(dilation_flag=setup_params['net_params']['dilation_flag']).to(torch.device('cuda'))
        ...
        self.net_weight = list(self.DilatedLoc.parameters())
    ...
    def train(self, printmodel=False):
        ...
        if printmodel:
            ...
            print('=' * 50)
            print(self.DilatedLoc)
            print("number of parameters: ", sum(param.numel() for param in self.DilatedLoc.parameters()))
        ...
        while self.start_epoch < self.endEpoch:
            ...
            for i in range(0, self.interval):
                ...
                p, xyzi_est, xyzi_sig = self.DilatedLoc(imgs_sim)

                loss_total = self.criterion.final_loss(p, xyzi_est, xyzi_sig, xyzi_gt, s_mask, locs)

                if self.start_epoch!= 0 and loss_total >2*self.recorder['cost_hist'][self.start_epoch]:
                    print("error")
                    #continue
                ...
                #torch.nn.utils.clip_grad_norm_(list(self.DilatedLoc.parameters()), max_norm=0.03, norm_type=2)
    ...
    def evaluation(self):
        ...
        with torch.set_grad_enabled(False):
            for batch_ind, (xemit, yemit, z, S, Nphotons, s_mask, gt) in enumerate(
                ...
                P, xyzi_est, xyzi_sig = self.DilatedLoc(imgs_sim, test=True)
        ...
        perf_dict, pred_lists = assess(res, gt_list, scale=self.EvalM.scale)
    ...
    def loadConfig(self):
        ...
        self.DilatedLoc.load_state_dict(checkpoint['state_dict'])
    ...
    def saveStatus(self,name=None):
        ...
        checkpoint = {"state_dict": self.DilatedLoc.state_dict(),
                      "optimizer": self.optimizer.state_dict(),
                      "epoch": self.start_epoch}
        if name:
            path_checkpoint = self.pathSave + name+'best_checkpoint.pth.tar'
            path_result = self.pathSave + name+'result.pkl'

        else:
            path_checkpoint = self.pathSave + 'best_checkpoint.pth.tar'
